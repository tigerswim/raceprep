...
import { TbClock, TbVs, TbCrystalBall, TbRun } from "react-icons/tb";
...
export const WebDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (isMounted) {
        console.warn(
          "[DASHBOARD] Loading timeout - continuing with minimal data",
        );
        setIsLoading(false);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (user) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(
              () => reject(new Error("User planned races query timeout")),
              3000,
            );
          });
          ...
          if (error) {
            console.warn(
              "[DASHBOARD] Database error loading planned races:",
              error,
            );
            // Use fallback data
            savedRaces = [
            ...
            console.log(
              "[DASHBOARD] Successfully loaded planned races:",
              data.length,
            );
            savedRaces = data.map((race: any) => ({
              id: race.id,
              name:
                race.external_races?.name || race.race_name || "Unknown Race",
          ...
          console.warn(
            "[DASHBOARD] Error or timeout loading saved races:",
            dbError.message,
          );
          // Use fallback data
          savedRaces = [
      ...
      const [racesResult, raceResultsResult] = await Promise.all([
        Promise.race([dbHelpers.races.getAll(), racesTimeoutPromise]).catch(
          () => ({ data: [], error: "timeout" }),
        ),
        user
          ? Promise.race([
              dbHelpers.raceResults.getUserResults(user.id),
              raceResultsTimeoutPromise,
            ]).catch(() => ({ data: [], error: "timeout" }))
          : Promise.resolve({ data: [], error: null }),
      ]);

      if (raceResultsResult.error) {
        console.warn(
          "[DASHBOARD] Error loading race results:",
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Unknown error occurred while loading dashboard";
  ...
  const calculatePerformanceStats = (raceResults: any[]): PerformanceStats => {
    ...
    const results: UserRaceResult[] = raceResults.map((result: any) => ({
      ...
      distance_type:
        result.races?.distance_type || result.distance_type || "sprint",
    ...
    const personalBest = results.reduce(
      (best, current) => {
    ...
    const thisYearRaces = results.filter(
      (r) => new Date(r.race_date).getFullYear() === currentYear,
    );
    ...
    if (results.length >= 6) {
      ...
      const recentAvg =
        recentRaces.reduce(
          (sum, race) => sum + parseTimeString(race.overall_time),
          0,
        ) / 3;
      const olderAvg =
        olderRaces.reduce(
    ...
    const transitionEfficiency = Math.max(
      0,
      Math.min(100, 100 - ((avgTransitionTime - 180) / 120) * 100),
    );
  ...
  return (
    <div
      className="bg-slate-900 relative h-full overflow-auto"
          ...
          {userRaceResults.length > 0 && (
            <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-6 shadow-2xl mb-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-white/60">
                    {performanceStats.latestRace?.race_name} •{" "}
                    {formatDate(performanceStats.latestRace?.race_date)}
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="text-center p-4 bg-white/5 rounded-xl">
                  <p className="text-2xl font-bold text-white font-mono mb-1">
                    {performanceStats.latestRace?.distance_type
                      ?.charAt(0)
                      .toUpperCase()}
                    {performanceStats.latestRace?.distance_type?.slice(1) ||
                      "Race"}
                  </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                <div className="text-lg font-bold mb-1 flex items-center justify-center gap-2">
                  <TbClock className="w-5 h-5" /> Add Race Result
                </div>
              ...
              <button
                onClick={() => {
                  if (userRaceResults.length >= 2) {
                    ...
                    const raceIds = userRaceResults
                      .slice(0, 2)
                      .map((result) => result.race_id || result.id);
              ...
              >
                <div className="text-lg font-bold mb-1 flex items-center justify-center gap-2">
                  <TbVs className="w-5 h-5" /> Compare Races
                </div>
                <div className="text-sm opacity-90">
                  {userRaceResults.length < 2
                    ? "Need 2+ races"
              ...
              >
                <div className="text-lg font-bold mb-1 flex items-center justify-center gap-2">
                  <TbCrystalBall className="w-5 h-5" /> Race Prediction
                </div>
                <div className="text-sm opacity-90">
                  {courses.length === 0
              ...
              >
                <div className="text-lg font-bold mb-1 flex items-center justify-center gap-2">
                  <TbRun className="w-5 h-5" /> Training
                </div>
                <div className="text-sm opacity-90">
