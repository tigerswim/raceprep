...
function formatPaceForCard(averageSpeed: number, type: 'swim' | 'bike' | 'run'): string {
...
function getFilteredWorkouts(workouts: WorkoutLog[], activityFilter: string, dateFilter: string): WorkoutLog[] {
  let filtered = [...workouts];

  // Filter by activity type
  if (activityFilter !== 'all') {
    filtered = filtered.filter(workout => workout.discipline === activityFilter);
  }
...
const TrainingScreenContent = React.memo(function TrainingScreenContent() {
  ...
  const [activityFilter, setActivityFilter] = useState<'all' | 'swim' | 'bike' | 'run'>('all');
  ...
  const loadStravaData = useCallback(async () => {
    try {
      ...
      const { data: userData, error: userError } = await dbHelpers.users.getProfile(user.id);
      ...
      const hasValidStravaToken = userData.strava_access_token &&
  ...
  const handleStravaDisconnect = async () => {
    ...
    try {
      ...
      const { error: updateError } = await dbHelpers.users.updateProfile(user.id, {
      ...
      const { error: deleteError } = await dbHelpers.trainingSessions.deleteAll();
      ...
      alert('Strava has been disconnected successfully. You can reconnect at any time.');
    ...
    try {
      ...
      if (activities && activities.length > 0) {
        ...
        const transformedActivities = activities
          ...
          .filter((activity: any) => !deletedActivities.includes(activity.id?.toString())) // Skip deleted activities
          .map((activity: any) => {
        ...
        alert(`Successfully synced ${transformedActivities.length} Strava activities!`);
      } else {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          if (sessionsResult.error) {
            console.error('[TRAINING_TAB] Database error:', sessionsResult.error);

            // If it's a table not found error, break the retry loop
            if (sessionsResult.error.code === 'PGRST106' ||
                sessionsResult.error.message?.includes('does not exist')) {
              console.warn('[TRAINING_TAB] Table does not exist - stopping retries');
              break;
            }
          ...
          if (sessionsResult.data && !sessionsResult.error) {
            ...
            const transformedWorkouts: WorkoutLog[] = sessionsResult.data.map((session: any) => {
              ...
              return {
                ...
                distance: session.distance ? parseFloat((session.distance * 0.000621371).toFixed(2)) : undefined, // Convert meters to miles
                distance_unit: 'miles' as 'miles' | 'km',
                intensity: (session.average_heartrate
                  ? getIntensityFromHeartRate(session.average_heartrate, session.max_heartrate)
                  : 'moderate') as 'easy' | 'moderate' | 'hard' | 'race_pace',
                notes: session.name || `${session.type.charAt(0).toUpperCase() + session.type.slice(1)} workout`, // Use activity name or generic description
          ...
          console.error('[TRAINING_TAB] Error loading training sessions (attempt', retryCount + 1, '):', error);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-4 shadow-xl">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div>
              <p className="text-xs text-white/60 uppercase tracking-wide">This Week</p>
              <p className="text-sm text-blue-300">Swimming</p>
            </div>
          </div>
          <div className="text-2xl font-bold text-white">
            {stravaConnected ? `${(weeklyStats.swim.distance / 1609.34).toFixed(1)} miles` : '3.2 miles'}
          </div>
          <p className="text-xs text-white/50">{stravaConnected ? `${weeklyStats.swim.sessions} sessions` : '4 sessions'}</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-4 shadow-xl">
          ...
          <div className="text-2xl font-bold text-white">
            {stravaConnected ? `${(weeklyStats.bike.distance / 1609.34).toFixed(0)} miles` : '85 miles'}
          </div>
          <p className="text-xs text-white/50">{stravaConnected ? `${weeklyStats.bike.sessions} sessions` : '3 sessions'}</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-4 shadow-xl">
          ...
          <div className="text-2xl font-bold text-white">
            {stravaConnected ? `${(weeklyStats.run.distance / 1609.34).toFixed(0)} miles` : '22 miles'}
          </div>
          <p className="text-xs text-white/50">{stravaConnected ? `${weeklyStats.run.sessions} sessions` : '5 sessions'}</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-4 shadow-xl">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div>
              <p className="text-xs text-white/60 uppercase tracking-wide">Training</p>
        ...
        <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-xl rounded-2xl border border-orange-400/20 p-4 md:p-6 shadow-xl">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex-1 min-w-0">
              <h3 className="text-lg md:text-xl font-bold text-white mb-1 md:mb-2">Connect Strava</h3>
              <p className="text-white/70 text-sm md:text-base">Import your training activities automatically from Strava to get real-time stats and progress tracking.</p>
            </div>
            <button
      ...
      {stravaConnected && (
        <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 backdrop-blur-xl rounded-2xl border border-green-400/20 p-4 md:p-6 shadow-xl">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold text-white mb-1">Strava Connected ✅</h3>
              <p className="text-white/70 text-sm">Your training data is synced from Strava. Click refresh to get the latest activities.</p>
            </div>
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
            ...
            >
              ...
              {(activityFilter !== 'all' || dateFilter !== 'all') && (
                <span className="bg-blue-500 text-white text-xs px-2 py-1 rounded-full">
                  {(activityFilter !== 'all' ? 1 : 0) + (dateFilter !== 'all' ? 1 : 0)}
                </span>
              )}
            </button>
            <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    { value: 'all', label: 'All Activities', icon: '🔄', color: 'bg-white/10' },
                    { value: 'swim', label: 'Swimming', icon: 'TbSwimming', color: 'bg-blue-500/20 border-blue-400/30 text-blue-400' },
                    { value: 'bike', label: 'Cycling', icon: 'TbBike', color: 'bg-orange-500/20 border-orange-400/30 text-orange-400' },
                    { value: 'run', label: 'Running', icon: 'TbRun', color: 'bg-green-500/20 border-green-400/30 text-green-400' }
                  ].map((type) => (
                    <button
                      key={type.value}
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    >
                      ...
                      <span className="sm:hidden">{period.label.split(' ')[period.label.split(' ').length - 1]}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-3">
          {getFilteredWorkouts(workoutLogs, activityFilter, dateFilter).slice(0, workoutsDisplayCount).map((workout) => {
            const performanceScore = calculatePerformanceScore(workout);
            const performanceColor = getPerformanceColor(performanceScore);

            return (
              <div key={workout.id} className="p-4 bg-white/5 rounded-xl hover:bg-white/10 cursor-pointer transition-colors border border-white/10" onClick={() => handleWorkoutClick(workout)}>
                <div className="flex items-start justify-between gap-4">
                  {/* Left side - Activity info */}
                  <div className="flex items-start space-x-3 flex-1 min-w-0">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      workout.discipline === 'swim' ? 'bg-blue-500/20' :
                      workout.discipline === 'bike' ? 'bg-orange-500/20' :
                      workout.discipline === 'run' ? 'bg-green-500/20' :
                      'bg-purple-500/20'
                    }`}>
                      <span className="text-lg">{
                        workout.discipline === 'swim' ? <TbSwimming className="w-4 h-4 text-blue-400" /> :
                        workout.discipline === 'bike' ? <TbBike className="w-4 h-4 text-orange-400" /> :
                        workout.discipline === 'run' ? <TbRun className="w-4 h-4 text-green-400" /> : <TbFlame className="w-4 h-4 text-purple-400" />
                      }</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        <h4 className="text-white font-semibold truncate">
                          {workout.notes && workout.notes !== `${workout.discipline.charAt(0).toUpperCase() + workout.discipline.slice(1)} workout`
                            ? workout.notes
                            : `${workout.discipline.charAt(0).toUpperCase() + workout.discipline.slice(1)} Workout`}
                        </h4>
                        <div className={`px-2 py-1 rounded-lg text-xs font-bold ${performanceColor} bg-white/10 flex-shrink-0`}>
                          {performanceScore}
                        </div>
                        {workout.trainer && (
                      ...
                      <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-white/70 mb-2">
                        <span className="whitespace-nowrap">📅 {new Date(workout.date).toLocaleDateString()}</span>
                        <span className="whitespace-nowrap font-mono">{Math.round((workout.duration_minutes || 0))}min</span>
                        {workout.distance && (
                          <span className="whitespace-nowrap">📏 {workout.distance.toFixed(1)} {workout.distance_unit}</span>
                        )}
                        {workout.average_speed && (
                          <span className="whitespace-nowrap flex items-center gap-1"><TbBolt className="w-3 h-3" /> {formatPaceForCard(workout.average_speed, workout.discipline)}</span>
                        )}
                        {workout.average_heartrate && (
                          <span className="whitespace-nowrap flex items-center gap-1"><TbHeart className="w-3 h-3" /> {Math.round(workout.average_heartrate)} bpm</span>
                        )}
                        {workout.average_watts && (
                          <span className="whitespace-nowrap">🔋 {Math.round(workout.average_watts)}W</span>
                        )}
                      </div>
                      ...
                      {workout.average_heartrate && (
                        <div className="text-xs text-white/60">
                          Zone: {getHeartRateZone(workout.average_heartrate, workout.max_heartrate)}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {workout.strava_activity_id && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleDeleteActivity(workout.id, workout.strava_activity_id);
                        }}
          ...
          {(() => {
            const filteredWorkouts = getFilteredWorkouts(workoutLogs, activityFilter, dateFilter);
            return filteredWorkouts.length > workoutsDisplayCount && (
              <div className="text-center pt-4">
                <button
                  onClick={() => {
                    setIsLoadingMore(true);
                    setTimeout(() => {
                      setWorkoutsDisplayCount(prev => Math.min(prev + 10, filteredWorkouts.length));
                ...
                >
                  {isLoadingMore ? (
                    ...
                    <>
                      <span>📂</span>
                      Load More ({filteredWorkouts.length - workoutsDisplayCount} remaining)
                    </>
                  )}
                </button>
              </div>
            ...
            if (workoutLogs.length === 0) {
              return (
                <div className="text-center py-8 text-white/60">
                  ...
                  <p className="text-sm mt-2">
                    {stravaConnected ? 'Try syncing your Strava activities.' : 'Connect Strava or log a workout manually.'}
                  </p>
                </div>
              );
              ...
              return (
                <div className="text-center py-8 text-white/60">
                  ...
                  <p className="text-sm mt-2">Try adjusting your filter settings or clearing them to see more results.</p>
                </div>
              );
            }
            return null;
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-white">Upcoming Training Events</h3>
          <button 
            ...
            <div key={event.id} className="p-4 bg-white/5 rounded-xl border border-white/10">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-white/70 text-sm mt-1">{event.description?.substring(0, 100)}...</p>
                  <div className="flex items-center space-x-4 mt-2 text-xs text-white/60">
                    <span className="flex items-center gap-1"><TbCalendar className="w-3 h-3" /> {new Date(event.date).toLocaleDateString()}</span>
                    {event.time && <span className="flex items-center gap-1"><TbClock className="w-3 h-3" /> {event.time}</span>}
                    {event.location && <span className="flex items-center gap-1"><TbMapPin className="w-3 h-3" /> {event.location}</span>}
                  </div>
                </div>
                <span className={`px-2 py-1 rounded-lg text-xs font-medium capitalize ${
                  event.event_type === 'clinic' ? 'bg-blue-500/20 text-blue-400' :
                  event.event_type === 'workshop' ? 'bg-green-500/20 text-green-400' :
                  event.event_type === 'group_training' ? 'bg-orange-500/20 text-orange-400' :
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Date</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Discipline</label>
                <select
                  value={newWorkout.discipline}
                  onChange={(e) => setNewWorkout({...newWorkout, discipline: e.target.value as any})}
                  className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-blue-400 focus:outline-none"
                >
                  <option value="swim" className="bg-slate-800">Swimming</option>
                  <option value="bike" className="bg-slate-800">Cycling</option>
                  <option value="run" className="bg-slate-800">Running</option>
                  <option value="brick" className="bg-slate-800">Brick Workout</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Duration (minutes)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Distance (optional)</label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Distance Unit</label>
                <select
                  value={newWorkout.distance_unit}
                  onChange={(e) => setNewWorkout({...newWorkout, distance_unit: e.target.value as 'miles' | 'km'})}
                  className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-blue-400 focus:outline-none"
                >
                  <option value="miles" className="bg-slate-800">Miles</option>
                  <option value="km" className="bg-slate-800">Kilometers</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Intensity</label>
                <select
                ...
                >
                  ...
                  <option value="race_pace" className="bg-slate-800">Race Pace</option>
                </select>
              </div>
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1">Feeling Rating (1-10)</label>
                <input
                  type="range"
                  min="1"
                  max="10"
                  value={newWorkout.feeling_rating}
                  onChange={(e) => setNewWorkout({...newWorkout, feeling_rating: parseInt(e.target.value)})}
                  className="w-full"
                />
                <div className="flex justify-between text-xs text-white/60 mt-1">
                  <span>1 (Terrible)</span>
                  <span className="text-white font-medium">{newWorkout.feeling_rating}</span>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <div className="space-y-2">
            {workoutLogs.map((workout) => (
              <div key={workout.id} className="p-3 bg-white/5 rounded-lg border border-white/10 hover:bg-white/10 cursor-pointer transition-colors" onClick={() => handleWorkoutClick(workout)}>
                <div className="flex items-center gap-3">
                  {/* Sport Icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="text-white font-medium text-sm leading-tight">{workout.notes || `${workout.discipline.charAt(0).toUpperCase() + workout.discipline.slice(1)} Workout`}</h4>
                      <span className="text-xs text-white/50 whitespace-nowrap ml-2">{new Date(workout.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                    </div>

                    <div className="flex items-center gap-3 text-xs text-white/70">
                      <span>{workout.duration_minutes}min</span>
                      {workout.distance && <span>{workout.distance} {workout.distance_unit}</span>}
                      ...
                      {workout.average_speed && (
                        <span>{calculatePace(workout.average_speed, workout.discipline)}</span>
                      )}

                      {/* Intensity badge */}
                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium capitalize ${
                        workout.intensity === 'easy' ? 'bg-green-500/20 text-green-400' :
                        workout.intensity === 'moderate' ? 'bg-blue-500/20 text-blue-400' :
                        workout.intensity === 'hard' ? 'bg-red-500/20 text-red-400' :
  ...
  const renderTrainingEvents = () => (
    <div className="space-y-6">
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        <h3 className="text-xl font-bold text-white mb-4">Upcoming Training Events</h3>
        <div className="space-y-4">
          ...
          {trainingEvents.length === 0 && (
            <div className="text-center py-8 text-white/60">
              ...
              <p className="text-sm mt-2">Check back later for new training opportunities!</p>
            </div>
          )}
        </div>
      </div>
  ...
  const renderTrainingArticles = () => (
    <div className="space-y-6">
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        <h3 className="text-xl font-bold text-white mb-4">Latest Training Tips & Articles</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {trainingArticles.map((article) => (
            <div key={article.id} className="p-4 bg-white/5 rounded-xl border border-white/10">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <span className={`px-2 py-1 rounded-lg text-xs font-medium capitalize ${
                      article.category === 'technique' ? 'bg-blue-500/20 text-blue-400' :
                      article.category === 'nutrition' ? 'bg-green-500/20 text-green-400' :
                      article.category === 'training' ? 'bg-orange-500/20 text-orange-400' :
                      article.category === 'mental' ? 'bg-purple-500/20 text-purple-400' :
                  ...
                  <h4 className="text-white font-semibold text-sm mb-2 line-clamp-2">{article.title}</h4>
                  <p className="text-white/70 text-xs mb-2 line-clamp-2">{article.excerpt}</p>
          ...
          {trainingArticles.length === 0 && (
            <div className="col-span-2 text-center py-8 text-white/60">
              ...
              <p className="text-sm mt-2">Check back later for new training tips!</p>
            </div>
          )}
        </div>
      </div>
  ...
  function calculateTrainingAnalytics(workouts: WorkoutLog[]) {
    ...
    const startOfThisWeek = new Date(now.getTime() - mondayOffset * 24 * 60 * 60 * 1000);
    startOfThisWeek.setHours(0, 0, 0, 0);

    // Get the start of last week (7 days before start of this week)
    const pastWeek = new Date(startOfThisWeek.getTime() - 7 * 24 * 60 * 60 * 1000);
    ...
    const monthlyVolume = Array(4).fill(0).map((_, weekIndex) => {
      // Calculate the start of each week (going backwards from most recent week)
      const weekStart = new Date(startOfThisWeek.getTime() - (weekIndex * 7 * 24 * 60 * 60 * 1000));
      ...
      const disciplineCounts = weekWorkouts.reduce((acc, w) => {
        acc[w.discipline] = (acc[w.discipline] || 0) + (w.duration_minutes || 0);
        return acc;
      }, {} as Record<string, number>);

      const primaryDiscipline = Object.entries(disciplineCounts)
    ...
    const totalDuration = currentMonthWorkouts.reduce((sum, w) => sum + (w.duration_minutes || 0), 0);
    const swimDuration = currentMonthWorkouts.filter(w => w.discipline === 'swim')
      .reduce((sum, w) => sum + (w.duration_minutes || 0), 0);
    const bikeDuration = currentMonthWorkouts.filter(w => w.discipline === 'bike')
      .reduce((sum, w) => sum + (w.duration_minutes || 0), 0);
    const runDuration = currentMonthWorkouts.filter(w => w.discipline === 'run')
      .reduce((sum, w) => sum + (w.duration_minutes || 0), 0);
    const brickDuration = currentMonthWorkouts.filter(w => w.discipline === 'brick')
      .reduce((sum, w) => sum + (w.duration_minutes || 0), 0);

    const distribution = totalDuration > 0 ? {
      swimming: Math.round((swimDuration / totalDuration) * 100),
      cycling: Math.round(((bikeDuration + brickDuration * 0.5) / totalDuration) * 100),
      running: Math.round(((runDuration + brickDuration * 0.5) / totalDuration) * 100)
    } : { swimming: 0, cycling: 0, running: 0 };

    // Calculate heart rate zones
    const workoutsWithHR = currentMonthWorkouts.filter(w => w.average_heartrate);
    const totalHRTime = workoutsWithHR.reduce((sum, w) => sum + (w.duration_minutes || 0), 0);
    ...
    const currentMonthStats = {
      ...
      totalHours: Math.round((currentMonthWorkouts.reduce((sum, w) => sum + (w.duration_minutes || 0), 0) / 60) * 10) / 10,
      avgPerformance: currentMonthWorkouts.length > 0
        ? Math.round((currentMonthWorkouts.reduce((sum, w) => sum + calculatePerformanceScore(w), 0) / currentMonthWorkouts.length) * 10) / 10
        : 0,
      totalDistance: {
        swim: Math.round((currentMonthWorkouts.filter(w => w.discipline === 'swim')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10,
        bike: Math.round((currentMonthWorkouts.filter(w => w.discipline === 'bike')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10,
        run: Math.round((currentMonthWorkouts.filter(w => w.discipline === 'run')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10
      }
    };
    ...
    const previousMonthStats = {
      ...
      totalHours: Math.round((previousMonthWorkouts.reduce((sum, w) => sum + (w.duration_minutes || 0), 0) / 60) * 10) / 10,
      avgPerformance: previousMonthWorkouts.length > 0
        ? Math.round((previousMonthWorkouts.reduce((sum, w) => sum + calculatePerformanceScore(w), 0) / previousMonthWorkouts.length) * 10) / 10
        : 0,
      totalDistance: {
        swim: Math.round((previousMonthWorkouts.filter(w => w.discipline === 'swim')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10,
        bike: Math.round((previousMonthWorkouts.filter(w => w.discipline === 'bike')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10,
        run: Math.round((previousMonthWorkouts.filter(w => w.discipline === 'run')
          .reduce((sum, w) => sum + (w.distance || 0), 0)) * 10) / 10
      }
    };
    ...
    const daysWithWorkouts = new Set(currentMonthWorkouts.map(w => new Date(w.date).toDateString())).size;
    const consistencyScore = Math.round((daysWithWorkouts / daysInCurrentMonth) * 100);
    ...
    const hourCounts = workoutsWithTime.reduce((acc, w) => {
    ...
    return {
      ...
      totalWeeklyHours: Math.round((pastWeekWorkouts.reduce((sum, w) => sum + (w.duration_minutes || 0), 0) / 60) * 10) / 10
    };
  }

  // Get month-over-month comparisons
  function getMonthlyComparison(current: number, previous: number): { change: number; isPositive: boolean; text: string } {
    if (previous === 0) {
      return {
  ...
  const renderAnalytics = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        <h3 className="text-xl font-bold text-white mb-4">Monthly Training Volume</h3>
        <div className="grid grid-cols-4 gap-3 mb-4">
          {analytics.monthlyVolume.map((weekData, index) => {
            const maxHours = Math.max(...analytics.monthlyVolume.map(d => d.hours), 1);
            // Ensure bars with data show meaningful height - minimum 60% for visibility
            const heightPercentage = weekData.hours > 0 ? Math.max((weekData.hours / maxHours) * 100, 60) : 0;

            const getColorForDiscipline = (discipline: string) => {
              switch (discipline) {
                case 'swim': return 'border-blue-400';
                case 'bike': return 'border-orange-400';
                case 'run': return 'border-green-400';
                case 'brick': return 'border-purple-400';
            ...
            const getColorForHeight = (discipline: string) => {
              switch (discipline) {
                case 'swim': return 'bg-blue-400';
                case 'bike': return 'bg-orange-400';
                case 'run': return 'bg-green-400';
                case 'brick': return 'bg-purple-400';
            ...
            return (
              <div key={index} className="text-center">
                <div className="text-xs text-white/60 mb-2">Week {4-index}</div>
                <div
                  className={`h-20 rounded-lg flex flex-col justify-end relative border overflow-hidden ${
                    weekData.hours > 0 ? getColorForDiscipline(weekData.primaryDiscipline) : 'bg-white/5 border-white/10'
                  }`}
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="text-white/70">Total this month: {analytics.monthlyVolume.reduce((sum, week) => sum + week.hours, 0).toFixed(1)}h</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          <h3 className="text-lg font-bold text-white mb-4">Training Distribution (This Month)</h3>
          <div className="space-y-3">
            {[
              { discipline: 'Swimming', percentage: analytics.distribution.swimming, color: 'bg-blue-500' },
              { discipline: 'Cycling', percentage: analytics.distribution.cycling, color: 'bg-orange-500' },
              { discipline: 'Running', percentage: analytics.distribution.running, color: 'bg-green-500' }
            ].map((item) => (
          ...
          {analytics.currentMonthStats.totalHours === 0 && (
            <div className="text-center py-4 text-white/60">
              ...
              <p className="text-xs">Start training to see your distribution!</p>
            </div>
          )}
        </div>

        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          <h3 className="text-lg font-bold text-white mb-4">Heart Rate Zones (This Month)</h3>
          <div className="space-y-3">
            {[
              { zone: 'Zone 1 (Recovery)', percentage: analytics.zoneDistribution.zone1, color: 'bg-blue-500' },
              { zone: 'Zone 2 (Aerobic)', percentage: analytics.zoneDistribution.zone2, color: 'bg-green-500' },
              { zone: 'Zone 3 (Tempo)', percentage: analytics.zoneDistribution.zone3, color: 'bg-yellow-500' },
              { zone: 'Zone 4-5 (Threshold+)', percentage: analytics.zoneDistribution.zone45, color: 'bg-red-500' }
            ].map((item) => (
              <div key={item.zone}>
                <div className="flex justify-between text-sm mb-1">
          ...
          {Object.values(analytics.zoneDistribution).every(v => v === 0) && (
            <div className="text-center py-4 text-white/60">
              <p className="text-sm">No heart rate data available</p>
              <p className="text-xs">Connect a heart rate monitor to see zone analysis!</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-blue-400 mb-2">{analytics.currentMonthStats.totalWorkouts}</div>
            <div className="text-white/70">Total Workouts</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.totalWorkouts, analytics.previousMonthStats.totalWorkouts).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.totalWorkouts, analytics.previousMonthStats.totalWorkouts).text}
            </div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-orange-400 mb-2">{analytics.currentMonthStats.totalHours}h</div>
            <div className="text-white/70">Training Time</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.totalHours, analytics.previousMonthStats.totalHours).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.totalHours, analytics.previousMonthStats.totalHours).text}
            </div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-green-400 mb-2">{analytics.currentMonthStats.avgPerformance}</div>
            <div className="text-white/70">Avg. Performance</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.avgPerformance, analytics.previousMonthStats.avgPerformance).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.avgPerformance, analytics.previousMonthStats.avgPerformance).text}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
        <h3 className="text-xl font-bold text-white mb-4">Distance Analysis (This Month vs Last Month)</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-400/20">
            ...
            <div className="text-2xl font-bold text-white mb-1">{analytics.currentMonthStats.totalDistance.swim} mi</div>
            <div className="text-sm text-white/60 mb-2">vs {analytics.previousMonthStats.totalDistance.swim} mi last month</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.totalDistance.swim, analytics.previousMonthStats.totalDistance.swim).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.totalDistance.swim, analytics.previousMonthStats.totalDistance.swim).text}
            </div>
          </div>
          ...
          <div className="bg-orange-500/10 rounded-xl p-4 border border-orange-400/20">
            ...
            <div className="text-2xl font-bold text-white mb-1">{analytics.currentMonthStats.totalDistance.bike} mi</div>
            <div className="text-sm text-white/60 mb-2">vs {analytics.previousMonthStats.totalDistance.bike} mi last month</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.totalDistance.bike, analytics.previousMonthStats.totalDistance.bike).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.totalDistance.bike, analytics.previousMonthStats.totalDistance.bike).text}
            </div>
          </div>
          ...
          <div className="bg-green-500/10 rounded-xl p-4 border border-green-400/20">
            ...
            <div className="text-2xl font-bold text-white mb-1">{analytics.currentMonthStats.totalDistance.run} mi</div>
            <div className="text-sm text-white/60 mb-2">vs {analytics.previousMonthStats.totalDistance.run} mi last month</div>
            <div className={`text-sm ${getMonthlyComparison(analytics.currentMonthStats.totalDistance.run, analytics.previousMonthStats.totalDistance.run).isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {getMonthlyComparison(analytics.currentMonthStats.totalDistance.run, analytics.previousMonthStats.totalDistance.run).text}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <div className="text-3xl font-bold text-white mb-2">{analytics.consistencyScore}%</div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <div className="text-3xl font-bold text-white mb-2">{analytics.currentMonthStats.avgPerformance}</div>
          <div className="text-white/70 text-sm">Average performance</div>
          <div className={`text-xs mt-1 ${analytics.currentMonthStats.avgPerformance >= 70 ? 'text-green-400' : analytics.currentMonthStats.avgPerformance >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
            {analytics.currentMonthStats.avgPerformance >= 70 ? 'Excellent' : analytics.currentMonthStats.avgPerformance >= 50 ? 'Good' : 'Needs improvement'}
          </div>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <div className="text-3xl font-bold text-white mb-2">
            {analytics.mostActiveHour !== null ? (
              analytics.mostActiveHour === 0 ? '12:00 AM' :
              analytics.mostActiveHour < 12 ? `${analytics.mostActiveHour}:00 AM` :
              analytics.mostActiveHour === 12 ? '12:00 PM' :
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-xl">
          ...
          <div className="text-3xl font-bold text-white mb-2">{Math.min(100, Math.round((analytics.currentMonthStats.totalHours / 20) * 100))}%</div>
          <div className="text-white/70 text-sm">Monthly target (20h)</div>
          <div className="mt-2">
            <div className="bg-white/10 rounded-full h-2">
              <div
                className="bg-green-500 h-2 rounded-full transition-all duration-1000"
                style={{ width: `${Math.min(100, Math.round((analytics.currentMonthStats.totalHours / 20) * 100))}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 p-6 pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
            <div className="mb-4 sm:mb-0">
              ...
              <p className="text-lg text-white/70">Track workouts, training plans, and progress</p>
            </div>
          </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800/90 backdrop-blur-xl rounded-2xl border border-white/10 w-full max-w-2xl max-h-[90vh] overflow-auto shadow-2xl">
              ...
              <div className="flex items-center justify-between p-6 border-b border-white/10">
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    selectedWorkout.discipline === 'swim' ? 'bg-blue-500/20' :
                    selectedWorkout.discipline === 'bike' ? 'bg-orange-500/20' :
                    selectedWorkout.discipline === 'run' ? 'bg-green-500/20' :
                    'bg-purple-500/20'
                  }`}>
                    <span className="text-lg">
                      {selectedWorkout.discipline === 'swim' ? <TbSwimming className="w-6 h-6" /> :
                       selectedWorkout.discipline === 'bike' ? <TbBike className="w-6 h-6" /> :
                       selectedWorkout.discipline === 'run' ? <TbRun className="w-6 h-6" /> :
                       <TbBolt className="w-6 h-6" />}
                    </span>
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-white">
                      {selectedWorkout.name || `${selectedWorkout.discipline.charAt(0).toUpperCase() + selectedWorkout.discipline.slice(1)} Workout`}
                    </h2>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {selectedWorkout.duration_minutes && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="text-2xl font-bold text-white">{selectedWorkout.duration_minutes}</div>
                  ...
                  {selectedWorkout.distance && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      ...
                      <div className="text-white/60 text-sm">{selectedWorkout.distance_unit || 'Miles'}</div>
                    </div>
                  )}
                  {selectedWorkout.feeling_rating && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="text-2xl font-bold text-white">{selectedWorkout.feeling_rating}/10</div>
                      <div className="text-white/60 text-sm">Feeling</div>
                    </div>
                  )}
                  {selectedWorkout.intensity && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="text-lg font-bold text-white capitalize">{selectedWorkout.intensity.replace('_', ' ')}</div>
                ...
                {(selectedWorkout.average_heartrate || selectedWorkout.average_speed || selectedWorkout.average_watts || selectedWorkout.total_elevation_gain) && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {selectedWorkout.average_heartrate && (
                        <div className="bg-red-500/10 rounded-lg p-3 border border-red-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-red-400 font-medium">Heart Rate</span>
                          </div>
                          <div className="text-xl font-bold text-white">{Math.round(selectedWorkout.average_heartrate)} bpm</div>
                          {selectedWorkout.max_heartrate && (
                            <div className="text-white/60 text-sm">Max: {Math.round(selectedWorkout.max_heartrate)} bpm</div>
                          )}
                          {selectedWorkout.average_heartrate && (
                            <div className="text-white/60 text-sm">
                              Zone: {getHeartRateZone(selectedWorkout.average_heartrate)}
                            </div>
                          )}
                        </div>
                      )}
                      {selectedWorkout.average_speed && (
                        <div className="bg-blue-500/10 rounded-lg p-3 border border-blue-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            <span>🚀</span>
                            <span className="text-blue-400 font-medium">Pace</span>
                          </div>
                          <div className="text-xl font-bold text-white">
                            {calculatePace(selectedWorkout.average_speed, selectedWorkout.discipline)}
                          </div>
                        </div>
                      )}
                      {selectedWorkout.average_watts && (
                        <div className="bg-yellow-500/10 rounded-lg p-3 border border-yellow-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            <span>🔋</span>
                            <span className="text-yellow-400 font-medium">Power</span>
                          </div>
                          <div className="text-xl font-bold text-white">{Math.round(selectedWorkout.average_watts)}W</div>
                        </div>
                      )}
                      {selectedWorkout.total_elevation_gain && selectedWorkout.total_elevation_gain > 0 && (
                        <div className="bg-green-500/10 rounded-lg p-3 border border-green-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            <TbMountain className="w-4 h-4" />
                            <span className="text-green-400 font-medium">Elevation</span>
                          </div>
                          <div className="text-xl font-bold text-white">{Math.round(selectedWorkout.total_elevation_gain * 3.28084)} ft</div>
                        </div>
                      )}
                      {selectedWorkout.average_cadence && (
                        <div className="bg-purple-500/10 rounded-lg p-3 border border-purple-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            <span>🦵</span>
                            <span className="text-purple-400 font-medium">Cadence</span>
                          </div>
                          <div className="text-xl font-bold text-white">
                            {Math.round(selectedWorkout.average_cadence)} {selectedWorkout.discipline === 'run' ? 'spm' : 'rpm'}
                          </div>
                        </div>
                      )}
                      {selectedWorkout.suffer_score && (
                        <div className="bg-orange-500/10 rounded-lg p-3 border border-orange-400/20">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-orange-400 font-medium">Training Stress</span>
                          </div>
                          <div className="text-xl font-bold text-white">{selectedWorkout.suffer_score} TSS</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  {selectedWorkout.trainer && (
                    <div className="bg-purple-500/10 rounded-lg p-3 border border-purple-400/20">
                      <div className="flex items-center gap-2">
                        <span>🏠</span>
                        <span className="text-purple-400 font-medium">Indoor Training</span>
                      </div>
                    </div>
                  )}

                  {selectedWorkout.sport_type && selectedWorkout.sport_type !== selectedWorkout.discipline && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="text-white/60 text-sm mb-1">Sport Type</div>
                      <div className="text-white font-medium">{selectedWorkout.sport_type}</div>
                    </div>
                  )}
                  ...
                  {selectedWorkout.kudos_count && selectedWorkout.kudos_count > 0 && (
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="flex items-center gap-2">
                        <span>👍</span>
                        <span className="text-white">{selectedWorkout.kudos_count} Kudos</span>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white/80">Synced from Strava</span>
                        </div>
                        <a
